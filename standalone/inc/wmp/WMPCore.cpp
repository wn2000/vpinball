...
{
    char szTmp[MAXSTRING];
    WideCharToMultiByte(CP_ACP, 0, pbstrURL, -1, szTmp, sizeof(szTmp), NULL, NULL);

    for (int i = 0; i < strnlen(szTmp, sizeof(szTmp)); i++) {
        if (szTmp[i] == '\\' || szTmp[i] == '/')
            szTmp[i] = PATH_SEPARATOR_CHAR;
    }
