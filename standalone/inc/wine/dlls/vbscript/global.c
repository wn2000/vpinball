...
static IUnknown *create_object(script_ctx_t *ctx, const WCHAR *progid)
{
    ...
    if (!wcsicmp(progid, L"Scripting.FileSystemObject")) {
        ...
        hres = vpinball_create_object(progid, cf, (IUnknown*)&obj);
    }
#endif
    if(FAILED(hres))
        return NULL;
...
static HRESULT Global_MsgBox(BuiltinDisp *This, VARIANT *args, unsigned args_cnt, VARIANT *res)
{
    ...
    if(SUCCEEDED(hres)) {
        ...
        snprintf(log_buffer, sizeof(log_buffer), "MsgBox: prompt=%s", buf);
        vpinball_log_info(log_buffer);
    }
#endif
...
static HRESULT Global_Eval(BuiltinDisp *This, VARIANT *arg, unsigned args_cnt, VARIANT *res)
{
    ...
    snprintf(log_buffer, sizeof(log_buffer), "Eval: enter - str=\"%s\"", buf);
    vpinball_log_debug(log_buffer);
    ...
    snprintf(log_buffer, sizeof(log_buffer), "Eval: exit - hr=0x%08x", hres);
    vpinball_log_debug(log_buffer);
...
static HRESULT Global_Execute(BuiltinDisp *This, VARIANT *arg, unsigned args_cnt, VARIANT *res)
{
    ...
    static char log_buffer[256];
    snprintf(log_buffer, sizeof(log_buffer), "Execute: enter - str=\"%s\"", buf);
    vpinball_log_debug(log_buffer);
    ...
    snprintf(log_buffer, sizeof(log_buffer), "Execute: exit - hr=0x%08x", hres);
    vpinball_log_debug(log_buffer);
...
static HRESULT Global_ExecuteGlobal(BuiltinDisp *This, VARIANT *arg, unsigned args_cnt, VARIANT *res)
{
    ...
    static char log_buffer[256];
    snprintf(log_buffer, sizeof(log_buffer), "ExecuteGlobal: enter - str=\"%s\"", buf);
    vpinball_log_debug(log_buffer);
    ...
    snprintf(log_buffer, sizeof(log_buffer), "ExecuteGlobal: exit - hr=0x%08x", hres);
    vpinball_log_debug(log_buffer);
