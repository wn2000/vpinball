...
static BOOL is_matching_key(const struct dictionary *dict, const struct keyitem_pair *pair, const VARIANT *key, DWORD hash)
{
    ...
    VARIANT* v2 = (VARIANT*)&pair->key;

    if (V_VT(v) == (VT_VARIANT|VT_BYREF))
        v = V_VARIANTREF(v);

    if (V_VT(v2) == (VT_VARIANT|VT_BYREF))
        v2 = V_VARIANTREF(v2);

    if (is_string_key(v) && is_string_key(v2)) {
        if (hash != pair->hash)
            return FALSE;

        return strcmp_key(dict, key, v2) == 0;
    }

    if ((is_string_key(v) && !is_string_key(v2)) ||
        (!is_string_key(v) && is_string_key(v2)))
        return FALSE;
#endif
...
static HRESULT WINAPI dictionary_get_HashVal(IDictionary *iface, VARIANT *key, VARIANT *hash)
{
    ...
    case VT_BSTR:
        V_I4(hash) = get_str_hash(get_key_strptr(v), dictionary->method);
        break;
    case VT_UI1|VT_BYREF:
    case VT_UI1:
        V_I4(hash) = get_num_hash(V_VT(v) & VT_BYREF ? *V_UI1REF(v) : V_UI1(v));
        break;
    case VT_I2|VT_BYREF:
    case VT_I2:
        V_I4(hash) = get_num_hash(V_VT(v) & VT_BYREF ? *V_I2REF(v) : V_I2(v));
        break;
    case VT_I4|VT_BYREF:
    case VT_I4:
        V_I4(hash) = get_num_hash(V_VT(v) & VT_BYREF ? *V_I4REF(v) : V_I4(v));
        break;
    ...
    case VT_DISPATCH:
    {
        IUnknown *src = (V_VT(v) & VT_BYREF) ? *V_UNKNOWNREF(v) : V_UNKNOWN(v);
        IUnknown *unk = NULL;
    ...
    case VT_DATE:
        return get_flt_hash(V_VT(v) & VT_BYREF ? *V_DATEREF(v) : V_DATE(v), &V_I4(hash));
    case VT_R4|VT_BYREF:
    case VT_R4:
        return get_flt_hash(V_VT(v) & VT_BYREF ? *V_R4REF(v) : V_R4(v), &V_I4(hash));
    case VT_R8|VT_BYREF:
    case VT_R8:
        return get_flt_hash(V_VT(v) & VT_BYREF ? *V_R8REF(v) : V_R8(v), &V_I4(hash));
    case VT_INT:
    case VT_UINT:
    case VT_I1:
    case VT_I8:
