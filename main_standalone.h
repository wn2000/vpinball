...
class ITypeInfoImpl : public ITypeInfo {
public: 
    virtual ULONG STDMETHODCALLTYPE AddRef() = 0;
    virtual ULONG STDMETHODCALLTYPE Release() = 0;
    ...
    STDMETHOD(Invoke)(PVOID pvInstance, MEMBERID memid, WORD wFlags, DISPPARAMS *pDispParams, VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr) {
      return S_OK;
    };

    STDMETHOD(GetDocumentation)(MEMBERID memid, BSTR *pBstrName, BSTR *pBstrDocString, DWORD *pdwHelpContext, BSTR *pBstrHelpFile) {
      
      return E_NOTIMPL;
    };

    STDMETHOD(GetDllEntry)(MEMBERID memid, INVOKEKIND invKind, BSTR *pBstrDllName, BSTR *pBstrName, WORD *pwOrdinal) {
      return S_OK;
    };
...
public:
   ...
   virtual const char* get_InternalClassName() {
    return "";
   }
...
public:
   ...
   ULONG InternalAddRef()
   {
      ...
      BSTR bstr;
      if (SUCCEEDED(this->GetDocumentation(MEMBERID_NIL, &bstr, NULL, NULL, NULL))) {
           name = wstring(bstr);
           SysFreeString(bstr);
      }

      PLOGI.printf("m_dwRef: %d -> %d %ls (%p)", m_dwRef - 1, m_dwRef, name.c_str(), (void*)this);
#endif
...
template <class T, const IID* piid /*= &__uuidof(T)*/,
          const GUID* plibid = &CAtlModule::m_libid, WORD wMajor = 1,
          WORD wMinor = 0, class tihclass = CComTypeInfoHolder>
class IDispatchImpl : public T, public ITypeInfoImpl {
public:
  virtual ULONG STDMETHODCALLTYPE AddRef() = 0;
  virtual ULONG STDMETHODCALLTYPE Release() = 0;
